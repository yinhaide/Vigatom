...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //一个窗口对应一个页面切换逻辑容器，不局限activity
        //可以用于fragment、viewgroup，dialog，presentation等容器
        new VigatomHelper() {
            @Override
            public VigatomBean getVigatomParam() {
                VigatomBean vigatomBean = new VigatomBean();
                //必须，根容器
                vigatomBean.setRootView(findViewById(R.id.viView)); 
                //必须，第一个显示的页面，需要继承ViView抽象类
                vigatomBean.setViViewClas(MainView.class);  
                //可选，对键盘的支持，可定制，需要继承ViKeyboardImpl            
                vigatomBean.setViKeyboardClass(SSDAKeyboard.class);  
                //可选，传入页面的窗体   
                vigatomBean.setWindow(getWindow()); 
                ////可选，是否设置自定义状态栏，需要存在window才支持                    
                vigatomBean.setVigatomStatusBus(true);                  
                return vigatomBean;
            }
        };
    }
